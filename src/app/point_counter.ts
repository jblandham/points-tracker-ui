...
// Declare these as possibly undefined to avoid strict TS errors if not provided by env
...
export class App implements OnInit {
  ...
  // Expose constant to template to fix TS2339 error
  ...
  private async initializeFirebase(): Promise<void> {
    ...
    try {
      // Check if globals are defined before usage
      const configString = (typeof __firebase_config === 'string' && __firebase_config !== 'undefined') ? __firebase_config : '{}';
      ...
      // Check if token exists before using
  ...
  private setupFirebaseListener(): void {
    ...
    // Using 'any' type for docSnap to avoid strict template error in local dev if types are missing
